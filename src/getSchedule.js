...
const getScheduleNoParameter = (scheduleTarget) => {
  ...
  weekDays.forEach((elemento) => {
    const array = [];
    const arrayAnimaisPerDay = data.species.filter((element) => element.availability.includes(elemento)) // const array = arrayAnimaisPerDay.reduce((acc, cur) => [...acc, cur.name], [])
